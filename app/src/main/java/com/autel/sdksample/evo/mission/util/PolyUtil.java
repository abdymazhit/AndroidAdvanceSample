...
public class PolyUtil {
    ...
    public static boolean isLocationOnEdge(com.amap.api.maps.model.LatLng point, List<com.amap.api.maps.model.LatLng> polygon, boolean geodesic,
                                           double tolerance) {
        return isLocationOnEdgeOrPath(point, polygon, true, geodesic, tolerance);
    }
    ...
    public static boolean isLocationOnPath(com.amap.api.maps.model.LatLng point, List<com.amap.api.maps.model.LatLng> polyline,
                                           boolean geodesic, double tolerance) {
        return isLocationOnEdgeOrPath(point, polyline, false, geodesic, tolerance);
    }
    ...
    private static boolean isLocationOnEdgeOrPath(com.amap.api.maps.model.LatLng point, List<com.amap.api.maps.model.LatLng> poly, boolean closed,
                                                  boolean geodesic, double toleranceEarth) {
        int idx = locationIndexOnEdgeOrPath(point, poly, closed, geodesic, toleranceEarth);
    ...
    private static int locationIndexOnEdgeOrPath(com.amap.api.maps.model.LatLng point, List<com.amap.api.maps.model.LatLng> poly, boolean closed,
                                                 boolean geodesic, double toleranceEarth) {
        int size = poly.size();
        if (size == 0) {
            return -1;
        ...
        double lat3 = toRadians(point.latitude);
        double lng3 = toRadians(point.longitude);
        com.amap.api.maps.model.LatLng prev = poly.get(closed ? size - 1 : 0);
        double lat1 = toRadians(prev.latitude);
        double lng1 = toRadians(prev.longitude);
        int idx = 0;
        if (geodesic) {
            for (com.amap.api.maps.model.LatLng point2 : poly) {
                double lat2 = toRadians(point2.latitude);
                double lng2 = toRadians(point2.longitude);
                if (isOnSegmentGC(lat1, lng1, lat2, lng2, lat3, lng3, havTolerance)) {
                    return Math.max(0, idx - 1);
