...
class DynamicTrackView : View {
    ...
    internal var canDrawReport = false
    ...
    fun setCanReport(canDraw : Boolean){
        this.canDrawReport = canDraw
    }
    internal var detectionTargetAreas: List<DetectionTargetArea>? = null
    fun setDetectionTargetArea(area: List<DetectionTargetArea>) {
        this.detectionTargetAreas = area
        invalidate()
    }
    var viewWith = 0
    var viewHeight = 0
    ...
    fun getText(detectionTargetArea: DetectionTargetArea): String {
        var value = ""
        var type = TargetType.find(detectionTargetArea.targetType)
        if(type == TargetType.LicensePlate){
//            var targetInfo = detectionTargetArea.targetInfo
//            targetInfo.substring()
            value = PlateNumberUtil.getPlateNumber(changeStringToArray(detectionTargetArea.targetInfo))
            return "LP:" + value
        }
        return type.name+","+value+","+detectionTargetArea.confidence+" "+ detectionTargetArea.targetType
    }

    fun changeStringToArray(list: MutableList<Int>): IntArray {
//        val aa = test.split(",".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
...
//        }
    ...
    override fun onDraw(canvas: Canvas) {
        super.onDraw(canvas)
        if (canDrawReport) {
            for (detectionTargetArea in this.detectionTargetAreas!!) {
                var left = detectionTargetArea.startX * width
                var top = detectionTargetArea.startY * height
                var widthArea = detectionTargetArea.width * width
                var heightArea = detectionTargetArea.height * height
                paint.color = Color.RED
                paint.textSize = 35f
                canvas.drawText(getText(detectionTargetArea), left + widthArea / 4, top + heightArea / 2, paint)
                paint.color = Color.BLUE
                Log.d("Dynamic","left:"+left+" right:"+right+" width:"+widthArea+" height:"+heightArea)
                canvas.drawRect(left, top, left + widthArea, top + heightArea, paint)
            }

        } else if (regDataReceived) {
            paint.color = color
...
//                        edgeBoxOverlay?.classification(mBitmap!!, regLeft, regTop, regRight, regBottom)
                ...
                if (null != callback) {
                    callback?.onDrawCompleted(getRegLeft(), getRegTop(), (regRight - regLeft), (regBottom - regTop))
                }
            }
    ...
    interface TrackCallback {
        fun onFingerDown()
        fun onDrawCompleted(x: Float, y: Float, width: Float, height: Float)
        fun onPointClick()
    }
